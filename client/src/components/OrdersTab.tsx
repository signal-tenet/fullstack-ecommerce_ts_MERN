...
const OrdersTab: React.FC = () => {
  ...
  const openDeleteConfirmBox = (order: string) => {
    setOrderToDelete(order);
    onOpen();
  };

  const onSetToDelivered = (order: string) => {
    dispatch(resetErrorAndRemoval());
    dispatch(setDelivered(order));
  };
  ...
  return (
    <Box>
      {error && (
        <Alert status='error'>
          <AlertIcon />
          <AlertTitle>Ups, we've got an error!</AlertTitle>
      ...
      {loading ? (
        ...
        <Box>
                    ...
                    <Tr key={order._id}>
                      ...
                      <Td>
                        <Flex direction='column'>
                          <Button
                            variant='outline'
                            onClick={() => openDeleteConfirmBox(order._id)}
                          >
                            <DeleteIcon mr='5px' />
                            Remove Order
                          </Button>
                          {!order.isDelivered && (
                            <Button
                              mt='4px'
                              variant='outline'
                              onClick={() => onSetToDelivered(order._id)}
                            >
                              <TbTruckDelivery />
          ...
          <ConfirmRemovalAlert
            ...
            onClose={onClose}
            cancelRef={cancelRef}
            itemToDelete={{ _id: orderToDelete, name: '' }}
